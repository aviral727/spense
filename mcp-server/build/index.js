...
async function sh(cmd, cwd = PROJECT_ROOT, timeoutMs = 360_000) {
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
const server = new Server({ name: "spense-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
// ── Tool definitions ──────────────────────────────────────────────────────────
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "get_context",
            description: "Returns CONTEXT.md — the definitive project context document. " +
        ...
        },
        {
            ...
            description: "Run './gradlew bundleRelease' to produce a signed AAB for Play Store upload. Takes ~3 minutes.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "build_release_apk",
            description: "Run './gradlew assembleRelease' to produce a signed APK for direct sideload/testing. Takes ~3 minutes.",
            inputSchema: { type: "object", properties: {} },
        },
        {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "bump_version": {
            const { versionCode, versionName } = args;
